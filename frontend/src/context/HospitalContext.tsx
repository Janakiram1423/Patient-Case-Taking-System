...
export const HospitalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addAppointment = (
    ...
    const todayAppointments = appointments.filter(a => a.appointment_date === appointmentData.appointment_date);
    const tokenNumber = todayAppointments.length + 1;
    ...
    addAuditLog(
      ...
      'Appointment',
      appointmentId,
      `Booked token #${tokenNumber} for ${newAppointment.patient_name} with ${newAppointment.doctor_name}`
    );
    showToast('success', 'Appointment Confirmed', `Token #${tokenNumber} assigned for ${newAppointment.patient_name}`);
